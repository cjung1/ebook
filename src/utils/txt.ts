...
export class TxTToEpubConverter{
    public async convert(options : TxT2EpubOptions) : Promise<ConversionResult> {
        ...
        const bookTitle = this.etractBookTitle(getBaseFilename(TxtFile.name));
        const fileName = `${bookTitle}.epub`;
        const fileHeader = txtContent.slice(0,1024);
        const authorMatch = 
        fileHeader.match(/[【\[]?作者[】\]]?[:：\s]\s*(.+)\r?\n/) ||
        fileHeader.match(/[【\[]?\s*(.+)\s+著\s*[】\]]?\r?\n/);
        let matchedAuthor = authorMatch ? authorMatch[1]! : providedAuthor || '';
        try{
    ...
    private async createEpub(chapters : Chapter[],metadata : Metadata) : Promise<Blob>{
        ...
        await zipWriter.add('mimetype',new TextReader('application/epub+zip'),zipWriterOptions);
    ...
    <container xmlns="urn:oasis:names:tc:opendocument:xmlns:container" version="1.0">
      <rootfiles>
        ...
        await zipWriter.add('META-INF/container.xml',new TextReader(containerXml),zipWriterOptions);
    ...
    <ncx xmlns="http://www.daisy.org/z3986/2005/ncx/" version="2005-1">
      ...
      <navMap>
        ...
        await zipWriter.add('toc.ncx',new TextReader(tocNcx),zipWriterOptions);
        const manifest = chapters.map(
            (_,index) => `<item id="chap${index + 1}" href="OEBPS/chapter${index + 1}.xhtml" media-type="application/xhtml+xml"/>`,
        ).join('\n')
        .trim();
        const spine = chapters.map(
            (_,index) => `<itemref idref="chap${index + 1}"/>`,
        ).join('\n')
        .trim();
        const css = `body { line-height: 1.6; font-size: 1em; font-family: 'Arial', sans-serif; text-align: justify; }
      p { text-indent: 2em; margin: 0; }`;

      await zipWriter.add('style.css',new TextReader(css),zipWriterOptions);
    ...
    const contentOpf = `<?xml version="1.0" encoding="UTF-8"?>
      ...
      await zipWriter.add('content.opf',new TextReader(contentOpf),zipWriterOptions);
    ...
    (txtContent : string,
        metadata : Metadata, 
        option : ExtractChapterOptions) : Chapter[] {
            ...
            if(language === 'zh'){
                ...
                const normalChapterPattern = chapterKeywords.map(
                    (k) => String.raw`${k}\s*(?:${numberPattern}|${dotNumberPattern})(?:[:.\-–—]?\s*${titlePattern})?`
                ).join('|');
                const prefacePattern = prefaceKeywords.map(
                    (k) => String.raw`${k}(?:[:.\-–—]?\s*${titlePattern})?`
                ).join('|');
            ...
            for(const segment of segments){
                ...
                let matches : string[] = [];
                for(const chapterRegexp of chapterRegexps){
                    const tryMatches = trimmedSegment.split(chapterRegexp);
    ...
    private etractBookTitle(filename : string) : string{
