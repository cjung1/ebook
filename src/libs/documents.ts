...
export interface BookDoc {
    ...
    rendition ?: {
        layout ?: 'pre-paginate' | 're-flowable';
        spread ?: 'auto' | 'none';
...
export class DocumentLoader{
    ...
    public async open() :Promise<{book : BookDoc,format : BookFormat}>{
        ...
        if(await this.isZip()){
            ...
            if(this.isCBZ()){
                ...
                const entry = (await entries).find((entry) => entry.filename.endsWith(`.${EXTS.FB2}`));
    ...
    private async makeZipLoader(){
        ...
        const map = new Map((await entries).map((entry) => [entry.filename,entry]));
